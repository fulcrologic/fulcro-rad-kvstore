...
(defn idents->value [ks env just-id-map? reference]
  (assert (some? reference) ["nil reference/value s/never be kept in db" (adaptor/instance-name-f ks env)])
  (let [recurse-f (partial idents->value ks env just-id-map?)]
    (cond
      (eql/ident? reference) (let [[table id] reference
                                   res (if just-id-map?
                                         {table id}
                                         (adaptor/read* ks env reference))]
...
(>defn read-outer
  ...
  (assert (satisfies? adaptor/KeyStore ks) ["Not a KeyStore" ks (keys (get env ::key-value/databases))])
  (let [ident (if (map? ident-or-m)
                ...
                (do
                  ...
                  ident-or-m))
        entity (adaptor/read* ks env ident)]
    (assert entity ["Could not find entity from ident" ident (adaptor/instance-name-f ks env)])
    (into {}
          (map (fn [[k v]]
                 (if (nil? v)
                   (do
