...
(>defn read-table
  ...
  output using `(mapv (fn [[table id]] {table id}))`
  This operation is not what Key Value stores are meant for. If you have not set ::key-value/table-kludge? to true
  then this operation will not be supported (MemoryKeyStore excepted). This function will likely be replaced with
  another/others that are more flexible and less relational/entity centric"
  [this env table]
